	...
	Describe("Table", func() {
		Describe("approximate offset test", func() {
			...
			It("Should be able to approximate offset of a key correctly", func() {
				...
				tr, err := NewReader(bytes.NewReader(buf.Bytes()), int64(buf.Len()), storage.FileDesc{}, nil, nil, o, nil, nil)
				Expect(err).ShouldNot(HaveOccurred())
		...
		Describe("read test", func() {
			Build := func(kv testutil.KeyValue) testutil.DB {
				...
				// Opening the table.
				tr, _ := NewReader(bytes.NewReader(buf.Bytes()), int64(buf.Len()), storage.FileDesc{}, nil, nil, o, nil, nil)
